...
vector<int> calculateMove(int myTime, int oppTime) {
    ...
    for (auto& c : finalCandidates) {
        ...
        // 6일차 2단계: 충돌 전략
        if (lastOppMoveCenter.first != -1) {
            int or1 = max(0, lastOppMoveCenter.first - 1);
            int oc1 = max(0, lastOppMoveCenter.second - 1);
            int or2 = min(9, lastOppMoveCenter.first + 1);
            int oc2 = min(16, lastOppMoveCenter.second + 1);
            auto oppRect = make_tuple(0, 0, or1, oc1, or2, oc2);

            if (isOverlappingWithOpponent(c, oppRect)) {
                if (oppTime >= 40) score -= 50;
                else if (oppTime <= 10) score += 50;
            }
        }

        // 7일차: 영역 우선 전략
        int areaScore = 0;
        if (center_r <= 3) areaScore += 10;
        if (center_r >= 7) areaScore -= 10;
        if (center_c >= 6 && center_c <= 10) areaScore += 5;
        if (oppTime >= 40) areaScore *= -1;
        score += areaScore;

        // 8일차: 사각형 형태 전략
        int height = r2 - r1 + 1;
        int width = c2 - c1 + 1;
        float ratio = (float)max(height, width) / min(height, width);
        int shapeScore = 0;
        if (ratio <= 1.2f)      shapeScore += 20;
        else if (ratio > 2.5f)  shapeScore -= 20;
        score += shapeScore;
