...
vector<int> calculateMove(int myTime, int oppTime) {
    ...
    for (auto& c : finalCandidates) {
        ...
        // 7일차: 영역 우선 전략
        int areaScore = 0;
        if (center_r <= 3) areaScore += 10;      // 상단: 공격 선호
        if (center_r >= 7) areaScore -= 10;      // 하단: 수비 선호
        if (center_c >= 6 && center_c <= 10) areaScore += 5;  // 세로 중심선
        if (oppTime >= 40) areaScore *= -1;      // 수비 시 방향 반전
        score += areaScore;
