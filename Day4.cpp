...
vector<int> calculateMove(int myTime, int oppTime) {
    ...
    for (const auto& cand : candidates) {
        auto [cnt, area, r1, c1, r2, c2] = cand;
    ...
    // Step 4-4: 가장 넓은 후보들 중, 중앙 충돌 가능성 높은 사각형 제외
    int maxArea = 0;
    ...
    // Step 4-5: 이전 사각형 중심과 너무 가까운 후보 제거
    ...
    // Step 4-6: 상대방 마지막 이동 중심과 거리가 너무 가까우면 제외
    ...
    if (lastOppMoveCenter.first == -1 || dist2 > 4)  // √4 = 2칸 이상 떨어지면 OK
...
for (auto& c : finalCandidates) {
    ...
    // 점수 계산
    score += 2 * cnt;                 // 숫자 많은 게 유리
    score -= area;                   // 면적이 작을수록 좋음
