...
private:
    ...
    vector<vector<bool>> opponentClaimed;
    vector<vector<bool>> myClaimed;
    pair<int, int> lastMyMoveCenter = {-1, -1};
    pair<int, int> lastOppMoveCenter = {-1, -1};
    vector<pair<int, int>> opponentHistory;
    ...
    int evaluateScore(int cnt, int area, int width, int height,
                      int center_r, int center_c,
                      pair<int, int> predDir,
                      double timeRatio) {
        // 기본 가중치
        int w_cnt = 2, w_area = 1, w_aspect = 3;
        int w_center = 5, w_oppDist = 1, w_predicted = 7;
        int w_nearMy = 3, w_sameDir = 2;

        // 전략 전환
        if (timeRatio > 1.5) {
            w_center = 10; w_predicted = 7; w_nearMy = 5;
        } else if (timeRatio < 0.7) {
            w_cnt = 3; w_area = 1; w_aspect = 0;
        }

        int aspectPenalty = abs(width - height);
        int centerPenalty = (center_r >= 4 && center_r <= 6 && center_c >= 7 && center_c <= 9) ? 1 : 0;
        int oppDist = 0;
        if (lastOppMoveCenter.first != -1) {
            int dx = center_r - lastOppMoveCenter.first;
            int dy = center_c - lastOppMoveCenter.second;
            oppDist = dx * dx + dy * dy;
        }
        int predictedThreat = isInPredictedPath(center_r, center_c, predDir) ? 1 : 0;

        // 내 점령지 인접 여부
        bool nearMy = false;
        for (int dr = -1; dr <= 1 && !nearMy; ++dr)
            for (int dc = -1; dc <= 1 && !nearMy; ++dc) {
                int r = center_r + dr, c = center_c + dc;
                if (r >= 0 && r < 10 && c >= 0 && c < 17 && myClaimed[r][c])
                    nearMy = true;
            }

        // 연속 방향 여부
        bool sameDir = false;
        if (lastMyMoveCenter.first != -1) {
            int dx = center_r - lastMyMoveCenter.first;
            int dy = center_c - lastMyMoveCenter.second;
            sameDir = (dx == predDir.first && dy == predDir.second);
        }

        int score = 0;
        score += w_cnt * cnt;
        score -= w_area * area;
        score -= w_aspect * aspectPenalty;
        score -= w_center * centerPenalty;
        score -= w_oppDist * oppDist;
        score -= w_predicted * predictedThreat;
        if (nearMy) score += w_nearMy;
        if (sameDir) score += w_sameDir;
...
public:
    ...
    vector<int> calculateMove(int myTime, int oppTime) {
        ...
        for (auto& c : filtered) {
            ...
            // 14일차 전략 ②: 이전 이동 방향과 같은 방향이면 보너스
            if (lastMyMoveCenter.first != -1) {
                int dx = center_r - lastMyMoveCenter.first;
                int dy = center_c - lastMyMoveCenter.second;
                if ((dx == predDir.first) && (dy == predDir.second)) {
                    score += 2;
                }
            }
        ...
        cout << "[DEBUG] 14일차 전략 선택: (" << r1 << "," << c1 << ") ~ (" << r2 << "," << c2 << "), 점수: " << bestScore << endl;
        cout.flush();
